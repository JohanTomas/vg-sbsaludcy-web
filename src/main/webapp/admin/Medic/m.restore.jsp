...
<head>
    ...
    <title>Restaurar Medicos | Sistema</title>
    <!-- CARPETA RESOURCE -->
...
<body>
                ...
                <div class="col-md-12">
                    ...
                    <div class="card">
                        <div class="card-header" data-background-color="blue">
                            <h4 class="title">Restaurar Medicos</h4>
                        </div>
                        <div class="card-content table-responsive">
                            <a href="admin/Medic/newmedic.jsp" class="btn btn-primary"><i class='fa fa-support'></i> Nuevo Medico</a>
                            <!-- Formulario de búsqueda -->
                            <div class="form-group">
                                <input type="text" id="searchInput" class="form-control" placeholder="Buscar por Nombre y Apellido, DNI o Dirección...">
                            </div>
                            <table id="medicrsTable" class="table table-bordered table-hover">
                                <thead>
                                ...
                                <c:forEach items="${medicList}" var="medic">
                                    <tr>
                                        <td style="text-align: center; vertical-align: middle;">${medic.id}</td>
                                        <td>${medic.name} ${medic.lastname}</td>
                                        <td style="text-align: center; vertical-align: middle;">${medic.document_dni}</td>
                                        <td style="text-align: center; vertical-align: middle;">${medic.day_of_birth}</td>
                                        <td style="text-align: center; vertical-align: middle;">${medic.address}</td>
                                        <td style="text-align: center; vertical-align: middle;">${medic.phone}</td>
                                        <td style="text-align: center; vertical-align: middle;">${medic.category_name}</td>
                                        <td style="width:140px; text-align: center; vertical-align: middle;">
                                            <a href="/Contrestorem?id=${medic.id}" class="btn btn-danger btn-xs restore-btn">Restaurar</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </table>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('.restore-btn').on('click', function(e) {
                ...
                swal({
                    title: "¿Deseas restaurar al Medico?",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
            ...
            const mensajedelete = '<%= request.getParameter("mensajerestore") %>';
            ...
            $('#searchInput').on('keyup', function() {
                ...
                $('#medicrsTable tbody tr').filter(function() {
    ...
    <script type="text/javascript">
        ...
        function exportToCSV() {
            const rows = document.querySelectorAll("#medicrsTable tbody tr");
            const csvData = [['Nombre completo', 'DNI', 'Fecha de Nacimiento', 'Dirección', 'Teléfono', 'Area']];
            rows.forEach(row => {
            ...
            link.setAttribute("download", "Medicos Inactivo.csv");
        ...
        function exportToXLS() {
            ...
            const rows = document.querySelectorAll("#medicrsTable tbody tr");
            const data = [['Nombre completo', 'DNI', 'Fecha de Nacimiento', 'Dirección', 'Teléfono', 'Area']];
            rows.forEach(row => {
                const cols = row.querySelectorAll('td');
                const rowData = [];
                cols.forEach((col, index) => {
            ...
            XLSX.writeFile(wb, 'Medicos Inactivo.xlsx');
        }
        ...
        async function exportTableToPDF() {
            ...
            doc.autoTable({
                head: [['Nombre completo', 'DNI', 'Fecha de Nacimiento', 'Dirección', 'Teléfono', 'Area']],
                body: tableData,
                startY: 10,
