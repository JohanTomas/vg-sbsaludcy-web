...
<head>
    ...
    <title> Restaurar Usuarios | Sistema</title>
    <!-- CARPETA RESOURCE -->
...
<body>
                ...
                <div class="col-md-12">
                    ...
                    <div class="card">
                        <div class="card-header" data-background-color="blue">
                            <h4 class="title"> Restaurar Usuarios</h4>
                        </div>
                        <div class="card-content table-responsive">
                            <a href="admin/Users/newuser.jsp" class="btn btn-primary"><i class='fa fa-user'></i> Nuevo Usuario</a>
                            <!-- Formulario de búsqueda -->
                            <div class="form-group">
                                <input type="text" id="searchInput" class="form-control" placeholder="Buscar por Nombre y Apellido, Usuario o Email...">
                            </div>
                            <br>
                            <table id="userrsTable" class="table table-bordered table-hover">
                                <thead>
                                ...
                                <c:forEach items="${userList}" var="user">
                                    <tr style="width:280px;">
                                        ...
                                        <td style="width:20px; text-align: center; vertical-align: middle;">
                                            <a href="/Contrestoreu?id=${user.id}" class="btn btn-danger btn-xs restore-btn">Restaurar</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </table>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('.restore-btn').on('click', function(e) {
                ...
                swal({
                    title: "¿Deseas restaurar al Usuario?",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
                    .then((willDelete) => {
                        if (willDelete) {
                            window.location.href = restoreUrl;  // Redirigir manualmente a la URL de eliminación
                        } else {
            ...
            const mensajerestore = '<%= request.getParameter("mensajerestore") %>';
            if (mensajerestore && mensajerestore !== 'null' && mensajerestore !== '') {
                swal({
                    title: mensajerestore,
                    icon: "success",
                    button: "Aceptar",
                });
            }
            ...
            $('#searchInput').on('keyup', function() {
                ...
                $('#userrsTable tbody tr').filter(function() {
    ...
    <script type="text/javascript">
        ...
        function exportToCSV() {
            const rows = document.querySelectorAll("#userrsTable tbody tr");
            const csvData = [['Nombre completo', 'Usuario', 'Email', 'Contraseña']];
            rows.forEach(row => {
            ...
            link.setAttribute("download", "Usuarios Inactivo.csv");
        ...
        function exportToXLS() {
            ...
            const rows = document.querySelectorAll("#userrsTable tbody tr");
            const data = [['Nombre completo', 'Usuario', 'Email', 'Contraseña']];
            rows.forEach(row => {
                const cols = row.querySelectorAll('td');
                const rowData = [];
                cols.forEach((col, index) => {
            ...
            XLSX.writeFile(wb, 'Usuarios Inactivo.xlsx');
        }
        ...
        async function exportTableToPDF() {
            ...
            doc.autoTable({
                head: [['Nombre completo', 'Usuario', 'Email', 'Contraseña']],
                body: tableData,
                startY: 10,
            ...
            doc.save('Usuarios Inactivo.pdf');
        }
    </script>
</body>
</html>